...
It is relevant to us as reaserchers for two reasons.

#### 1. Indexing and anonymizing sampling units
Unique, encrypted hashes can be created for any unique string in R,
We can also use hashes to publish reproducible code with annonymized individual or location information if there are privacy or conservation concerns. 

#### 2. Inventorying Files for Duplicates
...
#### 3. Inventorying and Renaming Files

We can also rename files with their unique hash, which is useful if we wish to link transcribed data in their most upstream digitized form to our CSVs used in analysis.

# Tutorials

## 1.  Using hashes to identify duplicate files.
...
We can append these hashes to our data frame, alongside filename, if desired.
...
```
# 3. Assigning a hash to an R object.
Hashes can be used to annonymize data in an encrypted form. 
